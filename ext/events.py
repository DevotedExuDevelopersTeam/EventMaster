...
class EventManagement(Cog):
        ...
        for row in data:
            ...
            if id not in event_data:
                event_data[id] = await self.bot.db.fetchrow(
                    """
                    SELECT
                        registration_channel_id AS channel_id,
                        registration_message_id AS message_id
                    FROM events
                    WHERE id = $1""",
                    id,
                )
            ...
            for child in view.children:
                if isinstance(child, disnake.ui.Button) and child.custom_id.endswith(str(row["role_id"])):
                    child.disabled = True
                    await msg.edit(view=view)
                    self.bot.log.info("Group %s is no longer joinable", child.custom_id)
                    break

        await self.bot.db.execute("UPDATE groups SET closed = TRUE WHERE start_time < $1", now)
    ...
    async def newevent(
        ...
        await inter.send(
            "Please add groups for event - send a sequence of UNIX timestamps separated "
            "by space in the following format:\n"
        ...
        for i, ts in enumerate(group_timestamps, 1):
            role = await inter.guild.create_role(name=f"{name} Event Participant G{i}".title())
        ...
        overwrites.update({r: disnake.PermissionOverwrite(read_messages=False) for r in roles})
        category = await inter.guild.create_category(f"{name.title()} Event")
        channel = await category.create_text_channel(f"{name.lower()}-registration", overwrites=overwrites)
        ...
        for i, (_, role_id, ts) in enumerate(groups, 1):
            view.add_item(disnake.ui.Button(label=f"Group {i}", custom_id=f"gr-{role_id}"))
    ...
    async def removeevent(self, inter: disnake.ApplicationCommandInteraction, id: str):
        id = uuid.UUID(id)
        exists = await self.bot.db.fetchval("SELECT EXISTS(SELECT 1 FROM events WHERE id = $1)", id)
        if not exists:
        ...
        data = await self.bot.db.fetchall("SELECT role_id FROM groups WHERE event_id = $1", id)
        for row in data:
            await inter.guild.get_role(row["role_id"]).delete()

        await inter.edit_original_response("Removing channel...")
        channel = self.bot.get_channel(
            await self.bot.db.fetchval("SELECT registration_channel_id FROM events WHERE id = $1", id)
        )
...
class TrophiesManagement(Cog):
    ...
    @commands.slash_command(name="remove_trophy", description="Removes the specified trophy")
    ...
    @commands.slash_command(name="give_trophy", description="Gives the trophy to a user")
    ...
    @commands.slash_command(name="take_trophy", description="Takes the trophy away from the user")
